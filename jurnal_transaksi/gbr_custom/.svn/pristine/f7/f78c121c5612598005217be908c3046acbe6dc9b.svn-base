...
STATUS_PENJUALAN = [('Clear','Clear'),('Under_Progress','Under Progress')]
...
class AccountMove(models.Model):
	...
	is_penjualan_clear = fields.Selection(STATUS_PENJUALAN, default="Under_Progress", string="Status Penjualan", tracking=True)
	...
	def button_penjualan_clear(self):
		if self.state == 'posted':
			self.write({'is_penjualan_clear':'Clear'})
		
		# return True
	#making a confirmation modal
	def action_confirm_clear(self):
		active_ids = self.env.context.get('active_ids')
		if not active_ids:
			return ''
		return {
			'name': _('Konfirmasi Clear'),
			'res_model': len(active_ids) == 1 and 'gbr.confirm.clear' or 'gbr.confirm.clear',
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_confirm_clear').id or self.env.ref('gbr_custom.view_gbr_confirm_clear').id,
	...
	def action_filter_data(self):
		...
		if not active_ids:
			return ''
		is_pembelian = False
		is_retur_pembelian = False
		is_debit_note_pembelian = False
		is_credit_note_pembelian = False
		is_penj_eceran = False
		for account_move in self:
			is_pembelian = account_move.is_pembelian
			is_retur_pembelian = account_move.is_refund
			is_debit_note_pembelian = account_move.is_debit_note_pembelian
			is_credit_note_pembelian = account_move.is_credit_note_pembelian
			is_penj_eceran = account_move.is_penj_eceran
		# print ('tststastadt',is_pembelian,is_retur_pembelian,is_credit_note_pembelian)
		if is_pembelian:
			return {
				'name': _('Filter Data'),
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_pemb_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_pemb_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
		if is_retur_pembelian:
			return {
				'name': _('Filter Data'),
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_retur_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_retur_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
		if is_debit_note_pembelian:
			return {
				'name': _('Filter Data'),
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_dn_p_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_dn_p_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}

		if is_credit_note_pembelian:
			return {
				'name': _('Filter Data'),
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_cn_p_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_cn_p_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
		if is_penj_eceran:
			return {
				...
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_penj_ecrn_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_penj_ecrn_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
	...
	def action_confirm_payment(self):
		...
		return {
			'name': _('Mengisi Tanggal Konfirmasi'),
			'res_model': len(active_ids) == 1 and 'gbr.confirm' or 'gbr.confirm',
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_confirm_am_form').id or self.env.ref('gbr_custom.view_gbr_confirm_am_form').id,
			'context': self.env.context,
			'target': 'new',
			'type': 'ir.actions.act_window',
		}
