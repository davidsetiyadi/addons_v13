...
class ProductProduct(models.Model):
	...
	def action_filter_data(self):
		...
		return {
			...
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_product_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_product_form').id,
