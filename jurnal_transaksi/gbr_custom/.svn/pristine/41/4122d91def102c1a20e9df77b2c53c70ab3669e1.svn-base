...
class WizardsFilter(models.TransientModel):
	...
	def default_get(self,fields):
		...
		elif self._context.get('from_saleorder'):
			sale_id = self.env['sale.order'].browse(active_ids)
			if sale_id.sale_2_id:
				purchase_obj = self.env['sale.order'].browse(sale_id.sale_2_id.id)
		
		elif self._context.get('from_rtr_penjualan'):
			sale_id = self.env['account.move'].browse(active_ids)
			if sale_id.penjualan_2_id:
				purchase_obj = self.env['account.move'].browse(sale_id.penjualan_2_id.id)
		else:
		...
		for data in purchase_obj:
			if data.partner_id:
				...
				query_penjualan_ids = """
					SELECT l.product_id,l.quantity,l.product_uom_id,l.price_unit,l.discount,l.price_total,l.name,t.group_1_id,t.group_2_id,t.group_3_id,
					p.barcode, p.default_code,p.category_uom_id from account_move_line as l inner join product_product p on p.id = l.product_id inner join product_template t on p.product_tmpl_id = t.id where l.move_id = %s 
					and exclude_from_invoice_tab = false""" %( data.id,)
				...
				if self._context.get('from_pembelian'):
					...
					product_ids = self.env.cr.execute(query_penjualan_ids)
					print ('productttttttttttttttttttsss',product_ids,data)
					results = self.env.cr.fetchall()
		...
		if len(filter_ids) >= 1:
			# if 
			if  self._context.get('from_rtr_penjualan'):
				date_order = data.invoice_date
			else:
	...
	def get_list_so2(self):		
		active_ids = self._context.get('active_ids')
		if not active_ids:
			return res	

		account_line_obj = self.env['sale.order.line']
		move = self.env['sale.order'].search([('id','=',active_ids[0])])[0]
		for data in self:
			new_lines = self.env['sale.order.line']
			for line in data.filter_ids:
				if line.is_confirm :					
					move.write({'order_line': [(0, 0, {
				                'product_id'	: line.product_id.id,
			...
			for line in data.filter_ids:
				if line.is_confirm :					
					move.write({'invoice_line_ids': [(0, 0, {
				                ...
				                'discount_2' 	: line.disc1,
				                'discount'		: line.disc1,
				                'account_id'	: self.env.company.account_retur_debit_id.id,
				                'price_unit'	: line.price,
				                'product_uom_id': line.uom_id.id,
				                'name'			: line.product_id.name,
					            })],})				
