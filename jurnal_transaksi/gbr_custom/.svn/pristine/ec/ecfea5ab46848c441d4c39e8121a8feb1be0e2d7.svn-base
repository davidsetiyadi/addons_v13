...
class SaleOrder(models.Model):
	...
	sale_2_id = fields.Many2one('sale.order', string="Quotation")
	sale_id = fields.Many2one('sale.order', string="Quotation 2")
	...
	def _amount_all_2(self):
		...
		for order in self:
			...
			for line in order.order_line:				
				factor = 1
				if line.product_uom:
					if line.product_uom.factor != 0 :
						factor = line.product_uom.factor

				product_qty += line.product_uom_qty / factor or 1
	...
	def action_confirm_payment(self):
		...
		return {
			...
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_confirm_so_form').id or self.env.ref('gbr_custom.view_gbr_confirm_so_form').id,
	...
	def action_filter_data(self):
		...
		for sale_order in self:
			is_quotation = sale_order.is_quotation
			is_order = sale_order.is_order
		...
		if is_quotation:
			return {
				...
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_filter_quotation_form').id or self.env.ref('gbr_custom.view_filter_quotation_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
		if is_order:
			return {
				...
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_filter_sale_form').id or self.env.ref('gbr_custom.view_filter_sale_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
