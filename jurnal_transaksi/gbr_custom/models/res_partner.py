...
class ResPartner(models.Model):
	...
	def action_filter_data(self):
		...
		for partner in self:
			customer = partner.is_customer
			is_value = partner.is_value
		if customer:
			return {
				'name': _('Filter Data'),
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_pelanggan_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_pelanggan_form').id,
		...
		if is_value: #vendor
			return {
				...
				'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
				'view_mode': 'form',
				'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_partner_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_partner_form').id,
				'context': self.env.context,
				'target': 'new',
				'type': 'ir.actions.act_window',
			}
