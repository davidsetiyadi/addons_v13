...
class account_payment(models.Model):
	...
	def _prepare_payment_moves(self):
		...
		for payment in self:
			...
			if self.env.context.get('disable_search', False):
				vendor_account = payment.partner_id._get_vendor_accounts()
				if payment.payment_type == 'outbound':					
					destination_account_id = vendor_account['deposit_pembelian'].id

				elif payment.payment_type == 'inbound':
					destination_account_id = vendor_account['deposit_penjualan'].id
	...
	def action_filter_data(self):
		...
		return {
			...
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_pv_form').id or self.env.ref('gbr_custom.view_gbr_filter_pv_form').id,
...
class GbrPaymentVoucher(models.Model):	
	...
	def action_confirm_payment(self):
		...
		return {
			...
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_confirm_form').id or self.env.ref('gbr_custom.view_gbr_confirm_form').id,
			'context': self.env.context,
			'target': 'new',
			'type': 'ir.actions.act_window',
		}
	def action_filter_data(self):
		...
		return {
			...
			'res_model': len(active_ids) == 1 and 'gbr.filter.data' or 'gbr.filter.data',
			'view_mode': 'form',
			'view_id': len(active_ids) != 1 and self.env.ref('gbr_custom.view_gbr_filter_data_form').id or self.env.ref('gbr_custom.view_gbr_filter_data_form').id,
			'context': self.env.context,
			'target': 'new',
			'type': 'ir.actions.act_window',
		}
